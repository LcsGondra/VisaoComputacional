...
def treinar_e_avaliar_svm():
    ...
    imagens, rotulos, origem = carregar_ou_extrair_dataset_hog(
        num_positivos=100, num_negativos=100
    )
    ...
    for r in range(2):
        for c in range(2):
            ax1.text(
                ...
                ha="center",
                va="center",
                color="black" if cm[r, c] < len(y_test) // 2 else "white",
                fontweight="bold",
            )

    ax2.imshow(cv2.cvtColor(img_resultado, cv2.COLOR_BGR2RGB))
    ax2.set_title(
        f"Deteccoes apos Janela Deslizante + NMS ({len(indices_nms)} objetos)",
        fontweight="bold",
    )
    ax2.axis("off")
